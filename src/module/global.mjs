 ...
 */
export async function deletedEffectRemovesItem(effect, effectName, itemName, descriptionPrefix, endMessage) {
    if (effect.name === game.i18n.localize(effectName)) {
        const actor = effect.parent;
        const item = actor.items.find(item => item.name === game.i18n.localize(itemName));
        if (item) {
            const itemDescription = item.system.description.value.replace(game.i18n.localize(descriptionPrefix), "").trim();
 ...
 */
export async function deleteEffectRemoveEffect(actor, effectToRemove, effectToIgnore, additionalEffectsToRemove) {
    ...
    const additionalEffectsToRemoveLocalized = additionalEffectsToRemove.map(effect => game.i18n.localize(effect));
    ...
    if (effect && actor.effects.find(i => i.name !== effectToIgnoreLocalized)) {
        ...
        const additionalEffects = actor.effects.filter(i => additionalEffectsToRemoveLocalized.includes(i.name));
