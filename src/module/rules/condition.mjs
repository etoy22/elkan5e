...
export function conditions() {
	...
	const NEW_CONDITIONS = {
		confused: {
			name: "Confused",
			reference:
				"Compendium.elkan5e.elkan5e-rules.JournalEntry.eS0uzU55fprQJqIt.JournalEntryPage.WJFtNc5UraHVrV5V",
		...
		cursed: {
			name: "Cursed",
			reference:
				"Compendium.elkan5e.elkan5e-rules.JournalEntry.sxKTtNPUrcDvMDFj.JournalEntryPage.Vpwu9GQC6HVNZFze",
		...
		dazed: {
			name: "Dazed",
			reference:
				"Compendium.elkan5e.elkan5e-rules.JournalEntry.eS0uzU55fprQJqIt.JournalEntryPage.0BYyVwipnS55gVFq",
			img: "modules/elkan5e/icons/conditions/dazed.svg",
		},
		drained: {
			name: "Drained",
			reference:
				"Compendium.elkan5e.elkan5e-rules.JournalEntry.eS0uzU55fprQJqIt.JournalEntryPage.ZnhMIMgPZv1QDxzZ",
			img: "modules/elkan5e/icons/conditions/drained.svg",
		},
		goaded: {
			name: "Goaded",
			reference:
				"Compendium.elkan5e.elkan5e-rules.JournalEntry.eS0uzU55fprQJqIt.JournalEntryPage.IVZ318d1P8WBcDxN",
			img: "modules/elkan5e/icons/conditions/goaded.svg",
		},
		hasted: {
			name: "Hasted",
			reference:
				"Compendium.elkan5e.elkan5e-rules.JournalEntry.eS0uzU55fprQJqIt.JournalEntryPage.8dnyv0szJi7dCz74",
		...
		silenced: {
			name: "Silenced",
			reference:
				"Compendium.elkan5e.elkan5e-rules.JournalEntry.eS0uzU55fprQJqIt.JournalEntryPage.F51xrE7Mj8VeM3b8",
			img: "modules/elkan5e/icons/conditions/silenced.svg",
		},
		siphoned: {
			name: "Siphoned",
			reference:
				"Compendium.elkan5e.elkan5e-rules.JournalEntry.eS0uzU55fprQJqIt.JournalEntryPage.SthB8javJuFySiBg",
			img: "modules/elkan5e/icons/conditions/siphoned.svg",
		},
		slowed: {
			name: "Slowed",
			reference:
				"Compendium.elkan5e.elkan5e-rules.JournalEntry.eS0uzU55fprQJqIt.JournalEntryPage.kkbgHooTzrtu4q8T",
			img: "modules/elkan5e/icons/conditions/slowed.svg",
		},
		weakened: {
			name: "Weakened",
			reference:
				"Compendium.elkan5e.elkan5e-rules.JournalEntry.eS0uzU55fprQJqIt.JournalEntryPage.iJT3cWvyTNBv1L5h",
		...
		surprised: {
			name: "Surprised",
			reference:
				"Compendium.elkan5e.elkan5e-rules.JournalEntry.eS0uzU55fprQJqIt.JournalEntryPage.QOZeW0m8RCdVg6UE",
	...
	const EFFECTS = {
		dead: [{ key: "attributes.hp.value", mode: 5, value: "0", priority: null }],
		blinded: [
		...
		silenced: [{ key: "flags.midi-qol.fail.spell.verbal", mode: 5, value: "1" }],
		stunned: [
			{
				key: "flags.midi-qol.grants.advantage.attack.all",
				mode: 5,
	...
	if (CONFIG.DND5E.statusEffects?.coverHalf)
		CONFIG.DND5E.statusEffects.coverHalf.img = `modules/elkan5e/icons/conditions/cover-half.svg`;
	if (CONFIG.DND5E.statusEffects?.coverThreeQuarters)
		CONFIG.DND5E.statusEffects.coverThreeQuarters.img = `modules/elkan5e/icons/conditions/cover-three-quarters.svg`;
	if (CONFIG.DND5E.statusEffects?.coverTotal)
		CONFIG.DND5E.statusEffects.coverTotal.img = `modules/elkan5e/icons/conditions/cover-full.svg`;
	CONFIG.DND5E.statusEffects.concentrating.img =
		"modules/elkan5e/icons/conditions/concentrating.svg";
...
export async function conditionsReady() {
	const conditionsSetting = game.settings.get("elkan5e", "conditions");
	CONFIG.statusEffects.find((effect) => effect.id === "dead").img =
		`modules/elkan5e/icons/conditions/dead.svg`;
	...
	if (conditionsSetting === "a" || conditionsSetting === "d") {
		...
		CONFIG.statusEffects = CONFIG.statusEffects.filter(
			(effect) => !UNUSED_CONDITIONS.includes(effect.id),
		);
	...
	CONFIG.statusEffects.find((effect) => effect.id === "diseased").img =
		"modules/elkan5e/icons/conditions/diseased.svg";
	CONFIG.statusEffects.find((effect) => effect.id === "hiding").img =
		"modules/elkan5e/icons/conditions/hiding.svg";
	CONFIG.statusEffects.find((effect) => effect.id === "dodging").img =
		"modules/elkan5e/icons/conditions/dodging.svg";
	...
	if (conditionsSetting === "a" || conditionsSetting === "b") {
		...
		if (version !== "modern") {
			const exhaustionEffect = CONFIG.statusEffects.find(
				(effect) => effect.id === "exhaustion",
			);
			...
			if (CONFIG.DND5E.conditionEffects?.halfMovement)
				CONFIG.DND5E.conditionEffects.halfMovement.delete("exhaustion-2");
			if (CONFIG.DND5E.conditionEffects?.halfHealth)
				CONFIG.DND5E.conditionEffects.halfHealth.delete("exhaustion-4");
			if (CONFIG.DND5E.conditionEffects?.noMovement)
...
export async function grapple(workflow) {
	...
	const grapplerRoll = await new Roll(
		"1d20 + @abilities.str.mod + @skills.ath.mod",
		grappler.getRollData(),
	).roll({ async: true });
	...
	for (const targetToken of game.user.targets) {
		...
		const targetRoll = await new Roll(
			`1d20 + @skills.${betterSkill}.mod`,
			target.getRollData(),
		).roll({ async: true });
