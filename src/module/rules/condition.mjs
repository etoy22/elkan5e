...
export function conditions() {
  ...
  CONDITION_ICONS.forEach((id) => {
    if (CONFIG.DND5E.conditionTypes[id]) {
      CONFIG.DND5E.conditionTypes[id].icon =
        `modules/elkan5e/icons/conditions/${id}.svg`;
    }
  });
...
export function icons() {
      ...
      changes: [
        {
          ...
          mode: 2,
          value:
            "turn=start, label=Confused Effect, macro=Compendium.elkan5e.elkan5e-macros.Macro.HW9jG0cdn6BmhzyE",
        },
      ],
    },
    {
    ...
    },
    {
      ...
      changes: [
        ...
        },
        {
          ...
          mode: 0,
          value:
            "Compendium.elkan5e.elkan5e-macros.Macro.4X80aHI9r8I9aSKG, preDamageApplication",
        },
      ],
    },
    {
    ...
    },
    {
      ...
      changes: [
        ...
        },
        {
          ...
          mode: 0,
          value:
            "Compendium.elkan5e.elkan5e-macros.Macro.1NtnoPvTQj1IEHCa, preDamageApplication",
        },
      ],
    },
    {
  ...
  STATUS_ICONS.forEach((id) => {
    if (CONFIG.statusEffects.find((effect) => effect.id === id)) {
      CONFIG.statusEffects.find((effect) => effect.id === id).img =
        `modules/elkan5e/icons/conditions/${id}.svg`;
    }
  });

  CONFIG.statusEffects.find((effect) => effect.id === "coverHalf").img =
    "modules/elkan5e/icons/conditions/cover-half.svg";
  CONFIG.statusEffects.find(
    (effect) => effect.id === "coverThreeQuarters",
  ).img = "modules/elkan5e/icons/conditions/cover-three-quarters.svg";
  CONFIG.statusEffects.find((effect) => effect.id === "coverTotal").img =
    "modules/elkan5e/icons/conditions/cover-full.svg";
...
export function dwarfResil() {
  Hooks.on("midi-qol.preCheckSaves", (workflow) => {
    ...
    if (actor.system.details.race?.toLowerCase().includes("dwarf")) {
      if (
        workflow.item.system.damage.parts.some((part) => part[1] === "poison")
      ) {
...
export function sturdy() {
  Hooks.on("midi-qol.preCheckSaves", (workflow) => {
    const actor = workflow.actor;
    const hasSturdyFeat = actor.items.some(
      (item) => item.name === "Sturdy" && item.type === "feat",
    );
    const isProneSave =
      workflow.item?.system.save?.ability === "dex" &&
      workflow.item?.name.includes("Prone");
