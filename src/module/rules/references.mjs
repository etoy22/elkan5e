...
export function creature() {
  try {
    ...
    Object.entries(CREATURE_REFS).forEach(([key, id]) => {
      CONFIG.DND5E.creatureTypes[key].reference =
        `Compendium.elkan5e.elkan5e-rules.JournalEntry.ZJX2hgglCq6NyNSD.JournalEntryPage.${id}`;
    });
  } catch (error) {
...
export function combatRef() {
  try {
    ...
    Object.entries(COMBAT_REFS).forEach(([key, id]) => {
      CONFIG.DND5E.rules[key] =
        `Compendium.elkan5e.elkan5e-rules.JournalEntry.C3b7Ref9xEVn34Gf.JournalEntryPage.${id}`;
    });
  } catch (error) {
...
export function conditionsRef() {
  try {
    ...
    CONDITIONS.forEach(({ key, id }) => {
      ...
      CONFIG.statusEffects.find((effect) => effect.id === "cursed").reference =
      ...
      CONFIG.statusEffects.find((effect) => effect.id === key).reference =
        reference;
    });
...
export function damageRef() {
  try {
    ...
    DAMAGE_TYPES.forEach(({ key, id }) => {
      CONFIG.DND5E.damageTypes[key].reference =
        `Compendium.elkan5e.elkan5e-rules.JournalEntry.C3b7Ref9xEVn34Gf.JournalEntryPage.${id}`;
    });
  } catch (error) {
...
export function spellCasting() {
  try {
    ...
    Object.entries(SPELL_REFS).forEach(([key, id]) => {
      CONFIG.DND5E.rules[key] =
        `Compendium.elkan5e.elkan5e-rules.JournalEntry.sxKTtNPUrcDvMDFj.JournalEntryPage.${id}`;
    });

    Object.entries(ITEM_PROPERTIES).forEach(([key, id]) => {
      CONFIG.DND5E.itemProperties[key].reference =
        `Compendium.elkan5e.elkan5e-rules.JournalEntry.sxKTtNPUrcDvMDFj.JournalEntryPage.${id}`;
    });
